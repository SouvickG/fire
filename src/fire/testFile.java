...
public class testFile {
    ...
    public static void main(String args[]) throws IOException, FileNotFoundException, Exception{
        ...
        File text = new File("TestDataInput.txt");
        //File annot = new File("2035_annotation.txt");
...
public void feat4(){
    ...
    public void writeFile() throws FileNotFoundException, IOException{
         ...
         File file = new File("TestFileCRF.txt");
         
         FileWriter fileWritter = new FileWriter(file.getName(),true);
    	 BufferedWriter bufferWritter = new BufferedWriter(fileWritter);
         
         for(int i=0;i<n;i++){
             bufferWritter.write(words[i]+"\t"+cap1[i]+"\t"+cap2[i]+"\t"+cap3[i]+"\t"+next3Wrd[i]+"\t"+prev3Wrd[i]+"\t"+currentWrd[i]+"\t"+length[i]+"\t"+hashPres[i]+"\t"+atPres[i]+"\t"+linkPres[i]+"\t"+isPunc[i]+"\t"+isSmiley[i]+"\t"+wrdHasSym[i]+"\t"+wrdHasDig[i]+"\t"+isNum[i]+"\t"+isEnglish[i]+"\t"+isBengali[i]+"\t"+isHindi[i]+"\t"+isGujrati[i]+"\t"+isMarathi[i]+"\t"+isMalayalam[i]+"\t"+isKanada[i]+"\t"+isTamil[i]+"\t"+isTelegu[i]+"\t"+isNE[i]+"\t"+isNEStan[i]+"\n");
